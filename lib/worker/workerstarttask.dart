...
class WorkerStartTaskPage extends StatefulWidget {
  final String taskid;
  final String latitude;
  final String longitude;

  const WorkerStartTaskPage(
      {Key key, this.taskid, this.latitude, this.longitude})
      : super(key: key);
  @override
  _WorkerStartTaskPageState createState() =>
      _WorkerStartTaskPageState(taskid, latitude, longitude);
}

class _WorkerStartTaskPageState extends State<WorkerStartTaskPage> {
  List<Step> steps;

  _WorkerStartTaskPageState(String taskid, String lat, String long);
  @override
  void initState() {
    getCurrentLoc();
    super.initState();
  ...
  checkLoc() async {
    ...
    if (isLocationEnabled) {
      ...
      double distanceInMeters = await Geolocator().distanceBetween(
          ...
          print('DIM : '+distanceInMeters.toString());
      ...
      if (response.statusCode == 200) {
        ...
        Future.delayed(
            ...
            () => _showDialog('Informasi',
                'Tugas dengan no : ' + widget.taskid.toString() + ' dimulai.'));
      } else {
